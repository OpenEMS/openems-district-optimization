...
while (Zeitschritt) {
  ...
  par(mfrow=c(2,1))
  plot(1:48, (Visualisierung$Pth_CHP1_TS1[1:48]/139.5*100), ylim=c(0,30), col=dunkelgruen, type="s", lwd=2, panel.first = grid(), xlab="Zeit in Stunden",ylab="Energie [kWh]", xaxt='n')
  axis(1, at = 1:48, labels = xAchse)
  lines(1:48, Visualisierung$Pth_TS1_Demth1[1:48] + Visualisierung$Pth_GaBo1_Demth1[1:48], col='black', type="s", lwd=2)
  lines(1:48, Visualisierung$Pth_GaBo1_Demth1[1:48], col='blue', type="s", lwd=2)
  legend("topright", legend=c("BHKW", "Wärmebedarf", "Gasboiler"), col=c(dunkelgruen, 'black', 'blue'), lty=1:1)
  title("Wärmesektor")
  ...
  Strombedarf <- Strombedarf_Bewohner + Strombedarf_EMob
  
  plot(1:48, Strombedarf, ylim=c(0,40), col='black', type="s", lwd=2, panel.first = grid(), xlab="Zeit in Stunden",ylab="Leistung [kW]", xaxt='n')
  axis(1, at = 1:48, labels = xAchse)
  lines(1:48, Erzeugung_PV, col='orange', type="s", lwd=2)
  lines(1:48, Erzeugung_BHKW, col=dunkelgruen, type="s", lwd=2)
  lines(1:48, Netzbezug*(1), col='red', type="s", lwd=3, lty=5)
  lines(1:48, rep(0,48), col='black', type="s", lwd=1)
  legend("topright", legend=c("Strombedarf", "PV", "BHKW", "Netzbezug"), col=c('black', 'orange', dunkelgruen, 'red'), lty=1:1)
  title("Stromsektor") 
  #
