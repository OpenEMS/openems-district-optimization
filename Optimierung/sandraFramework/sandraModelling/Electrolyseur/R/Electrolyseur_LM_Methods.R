...
#'LM.Constraints
#'
#'Build up the constraints for the linear modell of a Electrolyseur by binding
#'@importFrom Root LM.Constraints
#'
#'@export

setMethod("LM.Constraints", signature(object = "Electrolyseur"),function(object){

  if( length(maxP_el_minus(object))*
      length(minP_el_minus(object))*
      length(effMaxP_fuel_plus(object))*
      length(effMaxP_th_plus(object))*
      length(effMinP_fuel_plus(object))*
    ...
    M3<-LM.Transform_el_fuel(object)
    M4<-LM.Transform_el_th(object)
    ...
    #  price_fuel_minus(object)*colSums(P_fuel_plus(object))+M5@cost+M6@cost
  ...
  stop(paste("Attributes of", object@name, "are missing: minP_el_minus, effMaxP_fuel_plus, effMaxP_th_plus, effMinP_fuel_plus, effMinP_th_plus, price_maintenance, minRuntime, minDowntime,  coord
        are needed"))
})
