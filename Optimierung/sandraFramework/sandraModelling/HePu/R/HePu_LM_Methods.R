...
setMethod("LM.Constraints", signature(object = "HePu"),function(object){

  if(length(maxP_th_plus(object))*
     length(SourceTemp1(object))*
     ...
     length(SourceTemp1(object))*
     length(SourceTemp2(object))*
     length(SourceTemp_abstract(object))*
     length(COP1(object))*
     length(COP2(object))*
     length(coord(object))>0){
    ...
    M2<-LM.Transform_elth_th(object)
  ...
  stop(paste("Attributes of", object@name , "are missing: maxP_th_plus, SourceTemp1, SourceTemp2, SourceTemp_15min, COP1, COP2 and coord are needed"))
